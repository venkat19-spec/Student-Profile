package admin;

import misc.DBConnectivity;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import javax.servlet.http.Cookie;


@WebServlet(description = "Setup Warden Account", urlPatterns = { "/admin/WardenRegistration" })
public class WardReg extends HttpServlet {
	
	private static final long serialVersionUID = 5137308608826572778L;
	private static String admin_cookie = "f07bf50b0455c2346f8883d7697a158b703338dddc3c7b4cd33e2c1b85df0711";

	
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		
		boolean isAdmin = false;
		Cookie ck[] = req.getCookies();  
		for(int i=0;i<ck.length;i++){   
			if(ck[i].getName().equals("user_type") && ck[i].getValue().equals(admin_cookie)) {
				isAdmin = true;
			}
		}
		if(isAdmin) {
			req.getRequestDispatcher("/WEB-INF/admin/WardReg.jsp").forward(req, res);
		}
		else {
			req.setAttribute("error","Unauthorized access");
			req.getRequestDispatcher("../admin/LoginAdmin").forward(req, res);
		}
		
	}

	
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		
		boolean isAdmin = false;
		Cookie ck[] = req.getCookies();  
		for(int i=0;i<ck.length;i++){   
			if(ck[i].getName().equals("user_type") && ck[i].getValue().equals(admin_cookie)) {
				isAdmin = true;
			}
		}
		
		if(isAdmin) {
			
			String username = req.getParameter("fullname");
			String wardID = req.getParameter("username");
			String email = req.getParameter("email");
			String phoneNo = req.getParameter("phoneno");
			String password1 = req.getParameter("pass1");
			
			String block = req.getParameter("block");
			String gender = req.getParameter("gender");		
			
			int status = -1;
			Connection con;
			
			System.out.println("\nWarden Form Details to be written into database:\n");
			System.out.println(username);
			System.out.println(wardID);
			System.out.println(email);
			System.out.println(phoneNo);
			System.out.println(password1);
			System.out.println(block);
			System.out.println(gender);
			System.out.println("\n");
			
			try {
				
				con = DBConnectivity.initializeDatabase();
				PreparedStatement st = con.prepareStatement("INSERT INTO wardenregistration (WardID, WardenName, WardenEmail, WardenPhoneNo, WardenPassword, WardenBlock, WardenGender) VALUES (?, ?, ?, ?, ?, ?, ?);");
				
				st.setString(1, wardID);
				st.setString(2, username);
				st.setString(3, email);
				st.setString(4, phoneNo);
				st.setString(5, password1);
				st.setString(6, block);
				st.setString(7, gender);
				status = st.executeUpdate();
				
				con.close();
				st.close();
				
			} catch (ClassNotFoundException | SQLException e) {
				
				e.printStackTrace();
			}
			
			if(status>0) {
				getServletContext().setAttribute("message", "WARDEN account has been created for "+wardID+" successfully!!");
				res.sendRedirect("../admin/Notifications");
			}
			else {
				getServletContext().setAttribute("message", "Some SQL error happened, please try again!");
				res.sendRedirect("../admin/Notifications");
			}
			
		} 
		else {
			req.setAttribute("error","Unauthorized access");
			req.getRequestDispatcher("/WEB-INF/admin/adminLogin.jsp").forward(req, res);
		}
		
	}

}
